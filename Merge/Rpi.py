...
def play_siren_sound():
    try:
        # Replace 'siren.mp3' with the path to your siren sound file
        playsound('C:\datda manav\Manav\College\SMAPCA\Object_Detection_Files\Data\siren.mp3')
                ...
                if self.is_object_in_database(obj[1]):
                    if obj[1] not in self.detected_objects:
                        ...
                        if not self.scanned_product:
                            # Play the siren sound in a separate thread
                            sound_thread = threading.Thread(target=play_siren_sound)
                            sound_thread.start()

                            # Show the popup message
                            messagebox.showinfo("Object Detected", f"{obj[1]} detected. Please scan its barcode.")

                            # Stop the siren sound after 2 seconds
                            threading.Timer(2, sound_thread.join).start()
